...
class EfficientUNet(nn.Module):
    def __init__(self, out_channels=2, retrain_backbone=False):
        ...
        self.backbone: timm.models.efficientnet.EfficientNet = timm.create_model("tf_efficientnetv2_b0.in1k", pretrained=True, num_classes=0)
        ...
        new_conv = nn.Conv2d(1, 32, kernel_size=(3, 3), stride=(2, 2), padding=1, bias=False)
        ...
        self.stage1 = torch.nn.Sequential(
            torch.nn.LazyConvTranspose2d(112, 2, 2),    torch.nn.LazyBatchNorm2d(), torch.nn.ReLU()
        )
        self.stage2 = torch.nn.Sequential(
            torch.nn.LazyConv2d(112, 3, 1, 'same'),     torch.nn.LazyBatchNorm2d(), torch.nn.ReLU(),
            torch.nn.LazyConvTranspose2d(48, 2, 2),     torch.nn.LazyBatchNorm2d(), torch.nn.ReLU()
        )
        self.stage3 = torch.nn.Sequential(
            torch.nn.LazyConv2d(48, 3, 1, 'same'),      torch.nn.LazyBatchNorm2d(), torch.nn.ReLU(),
        ...
        self.stage5 = torch.nn.Sequential(
            ...
            torch.nn.LazyConvTranspose2d(12, 2, 2),     torch.nn.LazyBatchNorm2d(), torch.nn.ReLU()
        )
        self.stage6 = torch.nn.Sequential(
            torch.nn.LazyConv2d(12, 3, 1, 'same'),      torch.nn.LazyBatchNorm2d(), torch.nn.ReLU(),
            torch.nn.LazyConv2d(out_channels, 3, 1, 'same')
        )

        # Init lazy layer sizes
        self.eval()(torch.zeros(1, 1, 224, 224))
...
if __name__ == "__main__":
    ...
    for imgs, masks in dev_loader:
        ...
        for batch in range(main_args.batch_size):
            colored_mask1 = np.zeros((*masks[batch, 0].shape, 4))  # Initialize a transparent RGBA image
            colored_mask1[masks[batch, 0] == 1] = [1, 0, 0, 0.5]  # Red with 50% opacity
            colored_mask2 = np.zeros((*masks[batch, 1].shape, 4))  # Initialize a transparent RGBA image
            colored_mask2[masks[batch, 1] == 1] = [0, 1, 0, 0.5]  # Green with 50% opacity
